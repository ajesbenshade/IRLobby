...
export function ActivityCard({ activity, style, className = "" }: ActivityCardProps) {
  ...
  return (
    <Card 
      className={`swipe-card overflow-hidden ${className}`}
      style={style}
    >
      {/* If this is an injected ad, render AdCard instead */}
              ...
              <div className="flex items-center">
                {activity.participants && activity.participants.length > 0 && (
                  <div className="flex -space-x-2 mr-2">
                    {activity.participants.slice(0, 3).map((participant: any, index: number) => (
                      <div
                        key={index}
